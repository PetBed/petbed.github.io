/***************************************************
* ESP8266 Client (Updated with RGB LED Control)
* - Reads water level sensor (A0)
* - Reads digital rain sensor (D1)
* - Sends data as a JSON object via HTTP POST
 * - Receives LED state command in HTTP response
 * - Controls an RGB LED for illumination/status/emergency
***************************************************/
...
// NOTE: This assumes a common-cathode RGB LED. If you have a common-anode,
// the setLedColor logic will need to be inverted (255 - value).
...
// Non-blocking function to handle LED state in loop
void handleLed() {
