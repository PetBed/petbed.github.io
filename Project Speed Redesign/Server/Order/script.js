...
async function changeSection(index) {
	...
	for (let i = 0; i < subMenu[index].length; i++) {
      ...
      <div class="item-details">
        <p>${subMenu[index][i]}</p>
        <p>RM${subMenuPrice[index][i].toFixed(2)}</p>
...
function confirmOrder() {
	...
	confirmPrice.innerHTML = `RM ${totalPrice.toFixed(2)}`;
}
...
async function editItem(index) {
	...
	document.getElementById("edit-price").value = subMenuPrice[curMenu][index].toFixed(2);
