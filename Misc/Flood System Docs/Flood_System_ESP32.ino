/***************************************************
* ESP32 as HTTP Server, Blynk Client, and Alarm Clock (with GPS & Remote LED control)
* - Receives JSON POST data from ESP866
* - Shows date/time on an LED matrix (using NTPClient)
* - Sounds an alarm if the water level is too high
 * - Reads GPS location and sends it to web/Blynk
 * - Handles Blynk buttons for remote LED, emergency control, and alarm silence
 * - Sends LED commands back to ESP8266 in HTTP response
* - Shows status on a webpage and forwards to Blynk
* - Controls a local RGB LED to indicate flood severity
* - Tracks and displays duration of flood and rain events
***************************************************/
#define BLYNK_TEMPLATE_ID "TMPL6v2EAnw9W"
#define BLYNK_TEMPLATE_NAME "Flood System"
#define BLYNK_AUTH_TOKEN "Qc4jKa9k3JvtuGGm3_9ZNa0B3LBOT4eI"
...
float lastLatitude = 3.193809;
float lastLongitude = 113.057526;
bool manualLedOverride = false;
int manualLedState = 0;
bool emergencyLedActive = false;
bool heavyRainMode = false; // NEW: For "Heavy Rain" toggle
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n--- Flood Alert System Booting Up (Stable Version) ---");
  ...
  // --- REVISED: Setup for local RGB LED using standard pinMode ---
...
// REVISED: Blynk function to handle the alarm status switch
BLYNK_WRITE(V7) {
...
// NEW: Blynk function for Heavy Rain mode
BLYNK_WRITE(V10) {
  heavyRainMode = param.asInt() == 1;
  Serial.println("Heavy Rain mode toggled. State: " + String(heavyRainMode));
  updateBlynkRainStatus(); // Update display immediately
}
...
// --- REVISED: Manages the full lifecycle of the alarm ---
...
// --- REWRITTEN: Function to control the local RGB LED with 5 color steps ---
void updateRgbLed() {
  if (isAlarmActive) {
    ...
    if (progress >= 1.0) { // 100% -> Full Red
      r = 255; g = 0; b = 0;
    } else if (progress >= 0.75) { // 75% -> Orange-Red
      r = 255; g = 69; b = 0;
    } else if (progress >= 0.50) { // 50% -> Orange
      r = 255; g = 165; b = 0;
    } else if (progress >= 0.25) { // 25% -> Yellow-Orange
      r = 255; g = 200; b = 0;
    } else { // 0% -> Yellow
      r = 255; g = 255; b = 0;
    }

    // Set the color using standard analogWrite
...
// --- NEW: Updates the rain status string on Blynk ---
void updateBlynkRainStatus() {
  ...
  if (lastRainStatus == 0) {
    ...
    if (heavyRainMode) {
      Blynk.virtualWrite(V2, "Heavy Rain");
    } else {
...
// --- REVISED to handle flickering data ---
void handlePost() {
    ...
    if (jsonDoc.containsKey("water") && jsonDoc.containsKey("rain")) {
      ...
      if (lastWaterLevel > MIN_FLOOD_THRESHOLD) {
        ...
        if (floodConditionStartTime > 0) {
          ...
          if (floodStopTime == 0) {
            floodStopTime = millis(); // Mark the stop time ONCE.
      ...
      if (emergencyLedActive) {
        ...
        ledCommand = (manualLedState == 1) ? "ON" : "OFF";
      } else {
        ledCommand = isAlarmActive ? "ON" : "OFF";
      }

      Blynk.virtualWrite(V1, lastWaterLevel);
      updateBlynkRainStatus(); // REVISED: Use new function for rain status
...
// --- NEW HELPER: Formats milliseconds into HH:MM:SS ---
...
// --- REVISED to handle flickering data and reset correctly ---
void updateTimers() {
  // Flood Timer
...
void updateMatrixDisplay() {
  if (isAlarmActive) {
    ...
    // Not in alarm mode → just show time
    if (!showTimeInAlarm) {
...
// --- Functions below this point are unchanged ---
void handleGPS() {
